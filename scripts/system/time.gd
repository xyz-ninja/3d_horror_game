...
class CustomTimer:
	...
	var name = "unknown_timer" # also a dict key
...
func _ready():
	create_new_timer("button_timer",0.13)
	create_new_timer("mouse_wheel_timer",0.05)
...
func create_new_timer(_str_timer_name,_init_time):
	var new_timer = CustomTimer.new(last_timer_id, _init_time)
	last_timer_id += 1
